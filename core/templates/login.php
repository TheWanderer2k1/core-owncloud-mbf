...
?>
        ...
        <input style="border-radius: 8px" type="text" name="user" id="user" value="<?php p($_['loginName']); ?>"
            aria-label="<?php $_['strictLoginEnforced'] === true ? p($l->t('Login')) : p($l->t('Username or email')); ?>"
            <?php p($_['user_autofocus'] ? 'autofocus' : ''); ?> placeholder="<?php p($label); ?>" autocomplete="on"
    ...
    <div id="input-box" class="groupbottom<?php if (!empty($_['invalidpassword'])) {
        ...
        <div class="password-input-wrapper">
            <input style="border-radius: 8px" type="password" name="password" id="password" value=""
                <?php p($_['user_autofocus'] ? '' : 'autofocus'); ?> aria-label="<?php p($l->t('Password')); ?>"
                placeholder="<?php p($l->t('Password')); ?>" autocomplete="off" autocorrect="off" required>
            <button type="button" class="password-toggle-btn" id="password-toggle"
                aria-label="<?php p($l->t('Toggle password visibility')); ?>">
    ...
    <div class="submit-wrap">
        ...
        <button style="border: none;
							outline: none;
							border-radius: 8px;
							background: #fa709a;" type="submit" id="submit" class="login-button">
    ...
    <div class="remember-login-container">
        ...
        <input type="checkbox" name="remember_login" value="1" id="remember_login" class="checkbox checkbox--white"
            aria-label="<?php p($stayLoggedInText); ?>">
        <?php
			} else {
				?>
        <input type="checkbox" name="remember_login" value="1" id="remember_login" class="checkbox checkbox--white"
            checked="checked" aria-label="<?php p($stayLoggedInText); ?>">
        <?php
			} ?>
    ...
    <div class="forgot-password-link" style="margin-top: 20px; text-align: center;">
        <a href="https://auth-sso.mobifone.vn:8080/oauth/realms/sso-mobifone/login-actions/reset-credentials?client_id=application-demo"
            target="_blank" rel="noopener noreferrer" style="color: #fff; text-align: center;">
